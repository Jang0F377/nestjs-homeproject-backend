"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MattService = void 0;
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const matt_entities_1 = require("./entities/matt.entities");
let MattService = class MattService {
    constructor(mattModel) {
        this.mattModel = mattModel;
    }
    async returnAllMatt() {
        return await this.mattModel.find().sort({ priority: 1 }).exec();
    }
    async findOneMatt(id) {
        const matt = await this.mattModel.findById(id).exec();
        if (!matt) {
            throw new common_1.NotFoundException();
        }
        return matt;
    }
    async createMatt(newMatt) {
        const matt = new this.mattModel(newMatt);
        return matt.save();
    }
    async updateMatt(id, updatedMatt) {
        const existingMatt = await this.mattModel
            .findOneAndUpdate({ _id: id }, { $set: updatedMatt }, { new: true })
            .exec();
        return existingMatt;
    }
    async removeMatt(id) {
        const toRemove = await this.mattModel.findByIdAndDelete(id);
        return toRemove;
    }
    async clearMattDatabase() {
        return await this.mattModel.deleteMany();
    }
};
MattService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(matt_entities_1.Matt.name)),
    __metadata("design:paramtypes", [mongoose_2.Model])
], MattService);
exports.MattService = MattService;
//# sourceMappingURL=matt.service.js.map